...
if(isset($_POST['addproduk'])){
    ...
    if ($stocksekarang >= $qty){
        ...
        //stock ngga cukup
        echo '
        <script>alert("Stocknya barang tidak cukup");
        window.location.href="view.php?idp='.$idp.'"
        </script>
        ';
    }
...
if(isset($_POST['hapusprodukpesanan'])){
    ...
    $update = mysqli_query($c,"update produk set stock='$hitung' where idproduk='$idpr'"); //pudate stock
...
if(isset($_POST['editdatabarangmasuk'])){
    ...
    //cari tau dulu qty(jumlah) nya sekarang berapa
   ...
   if($qty >= $qtysekarang){
    // kalau input user lebih besar dari pada qty yang tercatat
    // hitung selisih
...
if(isset($_POST['editdetailpesanan'])){
   ...
   if($qty >= $qtysekarang){
    ...
    if($query1&&$query2){
        header('location:view.php?dip='.$idp);
    } else {
        echo '
        <script>alert("Gagal");
        window.location.href="view.php?idp='.$idp.'"
